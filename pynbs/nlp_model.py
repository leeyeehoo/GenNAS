...
from pynbs.embed_regularize import embedded_dropout
from pynbs.locked_dropout import LockedDropout
from pynbs.weight_drop import WeightDrop, ParameterListWeightDrop

from pynbs.custom_rnn import CustomRNN

import json
import numpy as np

class AWDRNNModelREG(torch.nn.Module):
    """Container module with an encoder, a recurrent module, and a decoder."""

    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, nreg = 32,
                 dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False,
                 recepie=None, verbose=True):
        super(AWDRNNModelREG, self).__init__()
...
#         self.encoder = torch.nn.Embedding(ntoken, ninp)
        ...
        self.decoder = torch.nn.Linear(ninp, nreg)
        self.encoder = torch.nn.Linear(nreg, ninp)
...
#         print(output.reshape(N*B,-1).shape)
